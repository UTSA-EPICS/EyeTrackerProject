...
# Keyboard settings (creates the keyboard windows and all the letters)
...
while True:
    ...
    keyboard[:] = (26, 26, 26)
    frames += 1
    # Converts the color image to grayscale for easier computations
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # Draw a white space for loading bar
    frame[rows - 50: rows, 0: cols] = (255, 255, 255)
    # Starts the "left" and "right" menu
    ...
    for face in faces:
        ...
        if select_keyboard_menu is True:
            # Detecting gaze ratio, to select Left or Right keyboard
            gaze_ratio_left_eye = get_gaze_ratio([36, 37, 38, 39, 40, 41], landmarks)
            gaze_ratio_right_eye = get_gaze_ratio([42, 43, 44, 45, 46, 47], landmarks)
            gaze_ratio = (gaze_ratio_right_eye + gaze_ratio_left_eye) / 2

            # Detects weather user is looking right
            print(gaze_ratio)
            if gaze_ratio <= 1.3:
                keyboard_selected = "right"
                keyboard_selection_frames += 1
                # If kept gaze on one side more than 30 frames, move to keyboard (more frames the slower)
                ...
                # Detects if the user is looking to the left
                keyboard_selected = "left"
                keyboard_selection_frames += 1
                # If Kept gaze on one side more than 30 frames, move to keyboard
                if keyboard_selection_frames == 30:
                    select_keyboard_menu = False
                    left_sound.play()
                    # Set frames count to 0 when keyboard selected
    ...
    # Shows the camera, virtual keyboard, and text board windows
